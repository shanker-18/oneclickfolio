...
export const themes = {
  // Professional Black & White Default Theme - Clean and Corporate
  default: {
    key: "default",
    name: "Professional Black & White",
    description: "Clean black and white theme with professional appearance and excellent visibility",
    category: "professional",
    colors: {
      primary: {
        light: "from-black to-gray-900",
        dark: "from-gray-800 to-gray-900"
      },
      secondary: {
        light: "from-gray-700 to-gray-900",
        dark: "from-gray-300 to-gray-500"
      },
      accent: "black",
      accentDark: "gray-400",
      background: {
      ...
      card: {
        light: "bg-white shadow-lg border border-gray-200 rounded-lg",
        dark: "bg-gray-800 shadow-xl border border-gray-600 rounded-lg"
      },
      cardHover: {
        light: "bg-gray-50 shadow-xl border-gray-300 transform hover:scale-105",
        dark: "bg-gray-700 shadow-2xl border-gray-500 transform hover:scale-105"
      },
      text: {
        primary: {
          light: "text-black",
    ...
    gradients: {
      header: "from-black via-gray-900 to-black", // Professional black header with great visibility
      skills: "from-gray-800 to-black",
      contact: "from-gray-700 to-gray-900"
    },
    animations: {
  ...
  coral: {
    ...
    colors: {
      ...
      accentDark: "pink-300",
      background: {
        light: "bg-gradient-to-br from-pink-50 to-teal-50",
        dark: "bg-gradient-to-br from-pink-900 to-teal-900"
      },
      card: {
    ...
    gradients: {
      header: "from-pink-500 via-rose-500 to-red-600",
      skills: "from-teal-500 to-cyan-600",
      languages: "from-cyan-500 to-teal-600"
    },
    animations: {
      transition: "all 0.35s ease",
      hover: "transform 0.25s ease",
...
export const themeCategories = {
  professional: ['default', 'sapphire', 'midnight', 'obsidian'],
  nature: ['emerald', 'coral'],
  warm: ['sunset', 'cherry'],
  luxury: ['royal', 'aurora'],
  modern: ['default', 'obsidian', 'sapphire'],
...
export const getThemeColors = (themeName, mode = 'light') => {
  ...
  console.log(`🎨 getThemeColors called with: ${themeName}, returning theme: ${theme.key}`);
  return {
    ...theme.colors,
    mode,
...
// Initialize user with clean default theme for first-time experience
export const initializeUserTheme = () => {
  ...
  const THEME_LOCK_KEY = 'portfolio-theme-locked';
  
  try {
    // Check if theme is locked to default
    const themeLocked = localStorage.getItem(THEME_LOCK_KEY);
    if (themeLocked === 'default') {
      console.log('🔒 Theme locked to default - forcing default theme');
      return {
        theme: 'default',
        mode: 'light',
        template: 'modern',
    ...
    if (!userInitialized) {
      // New user - assign clean default theme with modern template
      const themeConfig = {
        theme: 'default',
        mode: 'light', // Always start with light mode for clean appearance
        template: 'modern',
        timestamp: Date.now(),
        isRandomlyAssigned: false,
        isFirstTime: true
      ...
      // Don't lock theme - allow users to change it
      localStorage.removeItem(THEME_LOCK_KEY);
      
      console.log('🎨 Welcome! Your portfolio is ready with a clean, professional look.');
      
      return { ...themeConfig, isLocked: false };
    } else {
      // Existing user - load their preferences, but respect theme lock
      const preferences = loadThemePreference();
      if (themeLocked === 'default') {
    ...
    // Fallback to default clean theme
    return {
      theme: 'default',
      mode: 'light',
      template: 'modern',
...
// Function to lock/unlock theme
export const setThemeLock = (themeName) => {
  const THEME_LOCK_KEY = 'portfolio-theme-locked';
  if (themeName) {
    localStorage.setItem(THEME_LOCK_KEY, themeName);
    console.log(`🔒 Theme locked to: ${themeName}`);
  } else {
    localStorage.removeItem(THEME_LOCK_KEY);
...
// Function to check if theme is locked
export const isThemeLocked = () => {
  const THEME_LOCK_KEY = 'portfolio-theme-locked';
  return localStorage.getItem(THEME_LOCK_KEY) !== null;
};
...
// Utility function to reset user preferences (for testing/development)
export const resetUserPreferences = () => {
  if (typeof localStorage !== 'undefined') {
    localStorage.removeItem('portfolio-theme-preferences');
    localStorage.removeItem('portfolio-user-initialized');
    localStorage.removeItem('customColors');
    console.log('🔄 User preferences reset. Refresh the page to see the default theme.');
  }
};

// Export for development/testing - you can call this from browser console
if (typeof window !== 'undefined') {
  window.resetPortfolioPreferences = resetUserPreferences;
};
