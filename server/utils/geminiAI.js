...
class GeminiAIService {
  constructor() {
    ...
    this.genAI = new GoogleGenerativeAI(process.env.GEMINI_API_KEY || 'AIzaSyCm4kFn0KVhog6B6gC64WTANE6JsHWYHL0');
  ...
  async extractPortfolioData(resumeText, partialData = {}) {
    try {
      ...
      // Add timeout handling for portfolio extraction
      const timeoutPromise = new Promise((_, reject) => {
        setTimeout(() => reject(new Error('Portfolio extraction timeout after 30 seconds')), 30000);
      });
      ...
      // Log the AI prompt and output for debugging
      ...
      const llmFields = JSON.parse(cleanedText);
